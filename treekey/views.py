...
def first_node(request):
    ...
    return render_to_response('treekey/node.html', {'request': request, 'node':f, 'display':index_maker(), 'trail':trail_maker(f)})

# Returns a node or leaf depending on the child objects
def node(request, node_id):
    n = get_object_or_404(Node, pk=node_id)
    if n.has_children():
        return render_to_response('treekey/node.html', {'request': request, 'node':n, 'display':index_maker(), 'trail':trail_maker(n)})
    return render_to_response('treekey/leaf.html', {request: request, 'leaf':n, 'display':index_maker(), 'trail':trail_maker(n)})    
...
def species(request, species_id):
    ...
    n = s.node_set.all()[0];
    return render_to_response('treekey/species.html', {'request': request, 'species':s, 'display':index_maker(), 'trail':trail_maker(n)})
    
def browse(request):
    f = Node.objects.order_by('id')[0]
    species = Species.objects.all().order_by('common_name')
    return render_to_response('treekey/browse.html', {'request': request, 'species':species, 'display':index_maker(), 'trail':trail_maker(f)})
